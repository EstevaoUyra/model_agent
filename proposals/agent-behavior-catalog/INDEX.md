...
| ID | Thread | Dom | Role | Flags | Status | Key refs |
|----|--------|-----|------|-------|--------|----------|
| E1a | Evaluator leniency — acquits a discrepancy it has named | E | auditor | ⇄→E1b | mitigated | postmortem 2026-06-02; PR #5,#56; `faithfulness-critics-want-to-find-issues` |
| E1b | Perceptual blind spot — local curve defect passes | E | auditor | ⟳ | mitigated | PR #73; `curve-fidelity-audit-blindspot` |
| E2 | Leniency drift / grading own homework (heeger Vₑ) | E | auditor/critic | — | mitigated | `process-auditor-discriminates-drift-from-stuck`, `headline-result-gate` |
| E3 | Agent rubber-stamps its own tool/VLM output | E | any-with-tools | — | mitigated | `vlm-eye-is-arbiter-over-tools` |
| E4 | Sycophancy / flattery in agent output | E | any | — | mitigated | `dont-flatter-be-plain` |
| E5 | Self-certification (green tests ⇒ "done") | E | builder/auditor | ⟳ ↔M(D3) | recurring | `re-audit-after-every-model-change`, `headline-result-gate` |
| X1 | Over-routing to human for paper-resolvable findings | X | spec/process-auditor | ⟳ ↔X2 | mitigated | `paper-resolvable-findings-arent-human-routed`, `resolution-authority-ladder` |
| X2 | Escalation-ladder under-walked (self-resolvable→human) | X | orchestrator/resolver | ↔X1 | mitigated | `resolution-authority-ladder` |
| S1 | Organizer over-reach / do-it-myself reflex | C | orchestrator | ⟳ | recurring | `organizer-doesnt-implement-trust-the-process` |
| S2 | One shared decision resolved N× independently (saturation) | C | orchestrator+auditors | ⟳ | open | `saturation-is-the-genealogy-blocker` ("don't resolve 3×", "STILL BROKEN") |
| S3 | Workflow agents commit in wrong repo (parent vs submodule) | C | workflow/finalize | ⟳ | solved | `workflows-commit-in-submodule-not-parent`; PR #28 |
| S4 | Parallel work swallowed onto main / branch-discipline | C | orchestrator | ⟳✗ | solved | `branch-bumps-from-origin-main`; PR #47/#48,#58 |
| D1 | Process outran its docs (machinery ≠ description) | M | orchestrator/docs | ⟳ | mitigated | `process-outran-its-docs`; PR #55,#56,#70 |
| D2 | Generated-output drift (README re-fixed repeatedly) | M | README-gen/finalize | ⟳ | mitigated | PR #61–#65,#70,#72; `readable-human-facing-output` |
| D3 | Required step silently stops running (no coverage gate) | M | process/gate | ↔E5 | mitigated | `process-outran-its-docs`, `fix-mode-false-blocks-on-coverage`; PR #56,#66 |
| T1 | Ships unvalidated optimization to main (reverted ×2) | T | orchestrator | ✗⟳ →E5,S4 | solved | PR #45→#48→#50/#51→#59→#60; `digitization-cost-levers-ab` |
| T2 | Render sandbox lacks matplotlib (substrate) | T | builder/render | ⟳ | solved | `workflow-sandbox-lacks-matplotlib` *(Kind: environment/substrate)* |
| T3 | Acts on stale tool-model (snapshot / bad model-arg) | T | orchestrator/tooling | ✗ | solved | `workflow-tool-invocation-gotchas`, `workflow-agent-model-full-ids` |
...
*Pass-2/3 fan-out appends un-distilled threads (and `P#`/`U#` seeds) below this line.*
