...
const specDone = agent(
  ...
  { label: 'spec-extract', phase: 'Extract' })
...
const figResults = await pipeline(FIGURES,
  ...
  (fig) => agent(
    ...
    { label: `fig-extract:${fig}`, phase: 'Extract' }),
  ...
  async (_extracted, fig) => {
    ...
    for (let round = 1; round <= MAX_ROUNDS; round++) {
      ...
      await agent(
        ...
        { label: `digitize:${fig} r${round}`, phase: 'Digitization gate' })
      verdict = await agent(
        ...
        { label: `dig-audit:${fig} r${round}`, phase: 'Digitization gate', schema: DIG_VERDICT })
...
// ════════════════ BARRIER ── PHASE B — implement (paper-blind) ════════════════
phase('Implement')
await agent(
...
for (let round = 1; round <= MAX_ROUNDS; round++) {
  ;[faith, proc] = await parallel([
    () => agent(
      ...
      { label: `faith-audit r${round}`, phase: 'Verify', schema: FAITH_VERDICT }),
    () => agent(
      ...
      { label: `proc-audit r${round}`, phase: 'Verify', schema: PROC_VERDICT }),
  ])
  ...
  await agent(
    ...
    { label: `test-author r${round}`, phase: 'Verify' })
  ...
  if (contractBugs.length) {
    await agent(
      ...
      { label: `phaseA-fix r${round}`, phase: 'Verify' })
  }
  await agent(
    ...
    { label: `phaseB-fix r${round}`, phase: 'Verify' })
...
await agent(
  ...
  { label: 'state-update', phase: 'Report' })
